document.getElementById('notaForm').addEventListener('submit', async (event) => {
    ...
    const nota = document.getElementById('notaRaizes').value;
    
    try {
        const response = await fetch('/api/nota/raizes', {
            method: 'POST',
            headers: {
        ...
        document.getElementById('notaRaizes').value = ''; 
    } catch (error) {
...
async function atualizarMedia() {
    try {
        const response = await fetch('/api/media/raizes');
